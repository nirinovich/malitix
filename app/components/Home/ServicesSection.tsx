...
const SERVICE_OFFERINGS: ServiceOffering[] = [
  {
    ...
    description:
      "Équipe senior opérationnelle en 72h. Renforcez vos capacités avec nos experts dédiés.",
    highlight: "72h",
    icon: Cog,
  ...
  },
  {
    ...
    description:
      "Application web & mobile en 90 jours. Du concept au déploiement, une solution taillée pour votre métier.",
    highlight: "90 jours",
    icon: Globe,
  ...
  },
  {
    ...
    description:
      "iOS & Android native & cross-platform. Performances natives, expérience utilisateur premium.",
    icon: Smartphone,
  ...
  },
  {
    ...
    title: "Refonte SI",
    description:
      "Modernisation complète de votre système d'information. Migration, optimisation et pérennité.",
    icon: RefreshCcw,
    href: "/refonte-si",
  },
];
...
function ServiceCard({ service, layout, onClick }: ServiceCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative z-10 h-full flex ${isWide ? "flex-row items-center gap-8" : "flex-col"}`}
      >
        <div className={isWide ? "flex-1" : ""}>
          <div className="bg-[var(--brand-primary)] p-4 rounded-2xl w-fit mb-4 group-hover:scale-110 transition-transform duration-300">
            <Icon className="text-white" size={28} />
          </div>

          <h3
            className={`font-bold mb-3 text-[var(--text-primary)] ${isWide ? "text-2xl" : "text-xl"}`}
          >
            {service.title}
          </h3>
        ...
        <div
          className={`flex items-center justify-between ${isWide ? "flex-col items-end gap-4 self-end" : "mt-6"}`}
        >
          {service.highlight && (
            <span className="text-xs font-bold tracking-wider uppercase px-3 py-1 rounded-full bg-[var(--brand-primary)]/20 text-[var(--brand-primary)]">
              {service.highlight}
            </span>
          )}
          <div className="flex items-center gap-2 text-[var(--brand-primary)] font-semibold group-hover:gap-4 transition-all text-sm">
            Découvrir
            <ArrowRight size={18} />
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/2 right-0 w-96 h-96 rounded-full blur-3xl bg-[var(--brand-primary)]/10"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <p className="text-lg text-[var(--text-secondary)]">
            De la conception à la mise en production, nous accompagnons votre transformation
            digitale
          </p>
        </div>
