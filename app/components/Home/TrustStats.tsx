      ...
      >
        ...
        <div className="text-center max-w-4xl mx-auto mb-16 space-y-6 animate-on-scroll stagger-2">
          ...
          <p className="text-lg leading-relaxed text-[var(--text-secondary)]">
            Nous sommes une équipe passionnée qui croit que la technologie doit servir l'humain, pas
            l'inverse. Chaque projet est une opportunité de transformer vos défis en succès
            concrets. Que vous soyez une startup ambitieuse ou une entreprise établie, nous mettons
            notre expertise au service de votre vision pour créer des expériences digitales qui
            ...
            return (
              ...
              >
                ...
                <div className="relative z-10 space-y-4">
                  {/* Icon */}
                  <div className="bg-[var(--brand-primary)]/10 p-3 rounded-2xl w-fit group-hover:scale-110 transition-transform duration-300">
                    <Icon className="text-[var(--brand-primary)]" size={24} />
                  </div>
                ...
                {/* Corner decoration */}
                <div className="absolute top-4 right-4 w-16 h-16 border-t border-r rounded-tr-2xl transition-all duration-500 border-[var(--brand-primary)]/0 group-hover:border-[var(--brand-primary)]/30"></div>
              </div>
            );
          })}
        </div>
...
function CountUp({ end, suffix, isVisible }: { end: string; suffix: string; isVisible: boolean }) {
  ...
  // Format number
  const displayValue = numericEnd % 1 === 0 ? Math.floor(count) : count.toFixed(1);
